...
public class Quiz implements ValidacaoQuiz {
    ...
    public String exibirPerguntaQuiz() {
    ...
    public String pegarRespostaQuiz(){
        return quiz[i-1][1];
    }

    public int getQuizColumnsLength(){
        return quiz.length;
    }

    @Override
    public void validarResposta(String resposta) throws IllegalArgumentException {
        if(resposta == null){
            throw new IllegalArgumentException("A resposta do usuário não pode ser nula.");
        }
        if(resposta.equals(pegarRespostaQuiz())){
            jogador.atualizarScore(1);
        }
        System.out.println(pegarRespostaQuiz());
    }
